...
func (s *Store) GetRetailer(ctx context.Context, id domain.RetailerID) (domain.Retailer, error) {
...
func (s *Store) GetStore(ctx context.Context, id domain.StoreID) (domain.Store, error) {
...
func (s *Store) ListStores(ctx context.Context, retailerID domain.RetailerID) ([]domain.Store, error) {
	const q = `SELECT id, retailer_id, name, latitude, longitude, created_at
...
func (s *Store) UpsertRetailerProduct(ctx context.Context, rp domain.RetailerProduct) error {
	...
	var productID any = rp.ProductID
	if productID == nil {
		productID = nil
	}
...
func (s *Store) GetRetailerProduct(ctx context.Context, id domain.RetailerProductID) (domain.RetailerProduct, error) {
	...
	if productID != nil {
		...
		if err != nil {
			return domain.RetailerProduct{}, fmt.Errorf("persistence: parse product_id: %w", err)
		}
		rp.ProductID = &pid
	}
	if displayName != nil {
...
func (s *Store) ListRetailerProducts(ctx context.Context, retailerID domain.RetailerID) ([]domain.RetailerProduct, error) {
	...
	for rows.Next() {
		...
		if productID != nil {
			pid, err := domain.ParseProductID(*productID)
			if err != nil {
				return nil, fmt.Errorf("persistence: parse product_id: %w", err)
			}
			rp.ProductID = &pid
		}
		if displayName != nil {
...
// assigned UUID id.
func (s *Store) UpsertStoreProductOffer(ctx context.Context, offer domain.StoreProductOffer) (domain.StoreProductOfferID, error) {
	id := domain.NewStoreProductOfferID()
	const q = `INSERT INTO store_product_offer (id, store_id, retailer_product_id, currently_carried)
		...
		ON CONFLICT (store_id, retailer_product_id) DO UPDATE SET
			...
			updated_at = now()
		RETURNING id`
	var returnedID domain.StoreProductOfferID
	if err := s.db.QueryRow(ctx, q, id, offer.StoreID, offer.RetailerProductID, offer.CurrentlyCarried).Scan(&returnedID); err != nil {
		return domain.StoreProductOfferID{}, fmt.Errorf("persistence: upsert store_product_offer: %w", err)
	}
	return returnedID, nil
}

// GetStoreProductOffer fetches one store_product_offer by id.
func (s *Store) GetStoreProductOffer(ctx context.Context, id domain.StoreProductOfferID) (domain.StoreProductOffer, error) {
...
func (s *Store) ListStoreProductOffers(ctx context.Context, storeID domain.StoreID) ([]domain.StoreProductOffer, error) {
...
func (s *Store) ListRetailerProductOffers(ctx context.Context, retailerProductID domain.RetailerProductID) ([]domain.StoreProductOffer, error) {
...
func (s *Store) InsertPriceObservation(ctx context.Context, obs domain.PriceObservation) error {
	id, _ := uuid.NewV7()
	const q = `INSERT INTO price_observation (id, store_product_offer_id, observed_at, price, price_kind, source)
		VALUES ($1, $2, $3, $4, $5, $6)`
	if _, err := s.db.Exec(ctx, q, id.String(), obs.StoreProductOfferID, obs.ObservedAt, obs.Price, string(obs.PriceKind), obs.Source); err != nil {
...
func (s *Store) ListPriceObservations(ctx context.Context, offerID domain.StoreProductOfferID) ([]domain.PriceObservation, error) {
...
func (s *Store) GetLatestPriceObservation(ctx context.Context, offerID domain.StoreProductOfferID, kind domain.PriceKind) (domain.PriceObservation, error) {
	...
	if err := s.db.QueryRow(ctx, q, offerID, string(kind)).Scan(&obs.ID, &offerIDStr, &obs.ObservedAt, &obs.Price,
		(*string)(&obs.PriceKind), &obs.Source, &obs.CreatedAt); err != nil {
		if err == pgx.ErrNoRows {
			return domain.PriceObservation{}, fmt.Errorf("persistence: no price observation found for offer %s kind %q", offerID, kind)
		}
		return domain.PriceObservation{}, fmt.Errorf("persistence: get latest price_observation: %w", err)
	}
	obs.StoreProductOfferID, _ = domain.ParseStoreProductOfferID(offerIDStr)
	return obs, nil
}
...
func scanPriceObservations(rows pgx.Rows) ([]domain.PriceObservation, error) {
	...
	for rows.Next() {
		...
		var kind string
		var offerIDStr string
		if err := rows.Scan(&obs.ID, &offerIDStr, &obs.ObservedAt, &obs.Price,
...
func scanCurrentPrices(rows pgx.Rows) ([]domain.CurrentStoreProductPrice, error) {
	...
	for rows.Next() {
		...
		var kind string
		var offerIDStr string
		if err := rows.Scan(&offerIDStr, &p.StoreID, &p.RetailerProductID, &kind, &p.Price, &p.ObservedAt, &p.Source); err != nil {
			return nil, err
		}
		p.OfferID, _ = domain.ParseStoreProductOfferID(offerIDStr)
...
func (s *Store) PriceObservationsForProduct(ctx context.Context, retailerProductID domain.RetailerProductID) ([]domain.PriceObservation, error) {
...
func (s *Store) PriceObservationsForStore(ctx context.Context, storeID domain.StoreID) ([]domain.PriceObservation, error) {
	const q = `SELECT po.id, po.store_product_offer_id, po.observed_at, po.price, po.price_kind, po.source, po.created_at
		FROM price_observation po
		JOIN store_product_offer spo ON spo.id = po.store_product_offer_id
