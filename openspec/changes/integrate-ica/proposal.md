...
`/Users/andreas/dev/store-clients/ica-client`, structurally mirroring the existing, live
`/Users/andreas/dev/store-clients/willys-client`
...
- Track `/Users/andreas/dev/store-clients/ica-client`'s stabilization (build clean, tests
  green, live auth confirmed working against a real account) as an explicit prerequisite
  gate — this change's early tasks are verification, not new code, because the client isn't
  ready yet.
...
- No work on `/Users/andreas/dev/store-clients/ica-client` itself — that repo's stabilization
  is explicitly another agent's active work; this change only tracks its readiness as a gate.
...
- Affected code: none yet in this repo (gated on `ica-client` stabilization — see Non-Goals).
  ...
  paths.
- Depends on: `/Users/andreas/dev/store-clients/ica-client` (sibling repo, not owned by
  Spisordning) reaching a stable, tested, live-auth-verified state — tracked as this change's
  task 1, re-checked against `docs/research/ica-current-api.md` §5 before any adapter work
  begins.
