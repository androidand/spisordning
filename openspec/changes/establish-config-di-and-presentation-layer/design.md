...
migration needed) into one struct, validating required combinations at startup (e.g., `DATABASE_URL`
required for `serve`/`migrate`, optional for `plan`) instead of failing deep inside a client
constructor.
...
framework yet. Revisit if a third tiered retailer shows up.
...
2. Migrate `cmd/food-brain/adapters.go` to `config.Load()`, then `cmd/mcp-server/adapters.go` —
   one PR each, `go build`/`go test`/architecture-test gate both.
