...
- [x] 1.1 Confirm `implement-pantry-inventory`'s `InventoryLot` shape (ingredient/product,
...
- [x] 1.2 Read `research-barcode-scanning-devices`' proposal before starting — this change's
...
- [x] 2.1 Identify the exact model of the household's existing Bluetooth Brother label printer
...
- [x] 2.2 Test-print a minimal label against the household's actual printer once 2.1 identifies
      the API surface, to validate the connectivity/API assumption before designing further
      against it
      *Deferred — Brother P300BT currently loaned out; physical test deferred to implementation
      change. Research doc records this explicitly (see Open risks §1–3).*
- [x] 2.3 Survey other consumer/small-business thermal label printer options as fallback/
...
- [x] 2.4 Evaluate label stock cost/availability and freezer-durability (a label must survive
...
- [x] 2.5 Evaluate Bluetooth pairing reliability in a household kitchen setting for the specific
      printer on hand

## 3. Label content design

- [x] 3.1 Define label content fields: ingredient/product name, quantity (where known),
      stored-on date, best-before/use-by (where known), destination `InventoryLocation`
- [x] 3.2 Evaluate barcode vs. QR code for the scannable element — reliability at small label
...
- [x] 3.3 Design the label layout (a mockup, not implementation) balancing legibility of
...
- [x] 4.1 Define how a lot-reference code is generated (direct `InventoryLot` id encoding vs. a
...
- [x] 4.2 Design how this code is distinguishable from a GTIN at scan time (code
...
- [x] 4.3 Confirm this namespace does not violate `implement-pantry-inventory`'s "a barcode
...
- [x] 5.1 Design the optional print-on-`RecordPurchase` trigger (printer absence must not block
      or complicate a purchase being recorded — printing is strictly additive)
- [x] 5.2 Design an explicit "print label for this lot" action independent of purchase timing —
...
- [x] 5.3 Confirm the home-cooked/frozen-meal path end to end: `RecordPurchase` with
...
- [x] 6.1 Design the scan-label → resolve-`InventoryLot` → `RecordConsume`/`RecordDiscard`/
...
- [x] 7.1 Produce `docs/research/inventory-label-printing.md` recording findings, the printer
      hardware recommendation, label content/layout design, and the lot-reference barcode
      namespace design
- [x] 7.2 `openspec validate research-inventory-label-printing`
