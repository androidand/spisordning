...
Model the git-like household recipe hierarchy — RecipeFamily → RecipeVariant → RecipeRevision —
as an in-memory, persistence-free domain core with revision parentage held as a DAG. Encodes
immutability, single-family membership, acyclicity, and a manually-pinned default variant so the
schema and future persistence layer cannot drift from the domain invariants.
## Requirements
