...
Classify how the Directus admin workbench may (and may not) touch Spisordning's database:
Directus stays optional and removable, Spisordning remains the sole migration owner, and every
exposed collection is explicitly classified (`SAFE_DIRECT_CRUD`, `READ_ONLY`,
`DOMAIN_CONTROLLED`, or `HIDDEN`) so derived-state or invariant-backed tables cannot be
mutated through generic CRUD.
## Requirements
