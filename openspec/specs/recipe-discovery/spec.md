...
Bring external recipes into the household cookbook only through an explicit review step with
full source provenance, canonicalizing ingredient strings where possible and flagging (never
dropping) unresolvable ones — so the cookbook stays household-owned and reviewable rather than
absorbing arbitrary external content silently.
## Requirements
